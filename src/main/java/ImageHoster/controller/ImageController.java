...
public class ImageController {
    ...
    /*
     * Part A: Solution 1
     * Bug fixed when uploading images.
     * Replaced title with Id
     * In theory, two images can have the same title, but have different unique IDs.
     * ID in database is a primary key which doesn't allow which is a great candidate to differentiate images
     */
    ...
    public String editImage(@RequestParam("imageId") Integer imageId, Model model, HttpSession session, RedirectAttributes redirectAttrs) {
        ...
        //Bugged fixed for Part A Solution by creating a boolean helper function isImageOwner to verify the current user
    ...
    /*
    * method isImageOwner() was created as a boolean function to verify whether the user
    * logged into the current session matches the owner of the image.
    *
    */
    private Boolean isImageOwner(Integer imageId, HttpSession session){
        Image imageData = imageService.getImage(imageId); // Gets the image's details by querying it's ID
        Integer imageOwnerId = imageData.getUser().getId(); // Storing the Owner ID
        User currentUser = (User) session.getAttribute("loggeduser"); // Querying the current user's ID currently logged in
